...
dist2All_df<-function(x = NULL, id = NULL, dateTime = NULL, point.x = NULL, point.y = NULL, poly.xy = NULL, elev = NULL, parallel = FALSE, nCores = (parallel::detectCores()/2), dataType = "Point", lonlat = FALSE, numVertices = 4){
  ...
  if(is.data.frame(x) == FALSE & is.list(x) == TRUE){ #1/15 added the "is.data.frame(x) == FALSE" argument because R treats dataframes as lists.
    ...
    listBreak_dist.generator <-function(k,id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores, environmentTag = thisEnvironment){
      
      assign("listBreak", k, envir = environmentTag) #assign a "listBreak" object in the parent environment
      
      eval(expr = {
        
        dist.generator<-function(x,id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores){ #this is the same as dist.generator1 except that 
        ...
        listBreak.x <- x[[listBreak]] # pull the kth data frame from x.
        
        idVec1=NULL #added in the case that idVec1 isn't created when x isn't specified
        if(length(listBreak.x) == 0){ #This if statement allows users to input either a series of vectors (id, dateTime, point.x and point.y), a dataframe with columns named the same, or a combination of dataframe and vectors. No matter the input format, a table called "originTab" will be created.
          if(dataType == "point" || dataType == "Point" || dataType == "POINT"){
        ...
        if(length(listBreak.x) > 0){ #for some reason using an "else" statement would always result in an originTab table with 0 records...
          
          if(length(id) > 0){
            if(length(id) == 1 && is.na(match(id[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than id being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "id" values (i.e., if the ids in different list entries are different)
              listBreak.x$id <- listBreak.x[,match(id, names(listBreak.x))]
            }else{ #if length(id) > 1
              listBreak.x$id = id
            }
          ...
          idVec1 <- listBreak.x$id
          
          if(dataType == "point" || dataType == "Point" || dataType == "POINT"){
            
            if(length(point.x) > 0){
              if(length(point.x) == 1 && is.na(match(point.x[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than point.x being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "point.x" values (i.e., if the x-coordinate values in different list entries are different)
                listBreak.x$x <- listBreak.x[,match(point.x, names(listBreak.x))]
              }else{ #if length(point.x) > 1
                listBreak.x$x = point.x
              }
            }
            if(length(point.y) > 0){
              if(length(point.y) == 1 && is.na(match(point.y[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than point.x being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "point.x" values (i.e., if the x-coordinate values in different list entries are different)
                listBreak.x$y <- listBreak.x[,match(point.y, names(listBreak.x))]
              }else{ #if length(point.y) > 1
                listBreak.x$y = point.y
              }
            }
            xyFrame1<- data.frame(x = listBreak.x$x, y = listBreak.x$y)
          }
          
          if(dataType == "polygon" || dataType == "Polygon" || dataType == "POLYGON"){
            
            if(length(poly.xy) > 0){
              if(length(as.matrix(poly.xy)) == (numVertices*2) && length(which(is.na(match(as.matrix(poly.xy), names(listBreak.x)))) == TRUE) == 0){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than poly.xy being a matrix/dataframe of length(nrow(x)), it may be necessary to designate the colnames for intended coordinate values (i.e., if the xy-coordinate values in different list entries are different)
                xyFrame1<-listBreak.x[,match(poly.xy,names(listBreak.x))]
              }else{
                #x[,2:(1 + length(poly.xy))] = poly.xy
                xyFrame1<- data.frame(poly.xy)
              }
            }else{ #if length(poly.xy == 0)
              xyFrame1 <-  listBreak.x[,(match("point1.x", names(listBreak.x))):((2*numVertices) + (match("point1.x", names(listBreak.x)) -1))] #if there is no poly.xy input, the code assumes the input is output from referencePointToPolygon function, and therefore, the first point of interest would be "point1.x"
            }
          ...
          if(length(dateTime) > 0){
            ...
            if(length(dateTime) == 1 && is.na(match(dateTime[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than point.x being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "point.x" values (i.e., if the x-coordinate values in different list entries are different)
              listBreak.x$dateTime <- listBreak.x[,match(dateTime, names(listBreak.x))]
            }else{ #if length(dateTime) > 1
              listBreak.x$dateTime = dateTime
            }
            
          }
          dateTimeVec<-listBreak.x$dateTime
        ...
        if(length(elev) > 0){
          elev.calc <-TRUE 
          if(length(elev) == 1 && is.na(match(elev[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than elev being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "elev" values (i.e., if the elevs in different list entries are different)
            originTab$elev <- listBreak.x[,match(elev, names(listBreak.x))]
          }else{ #if length(elev) > 1
            originTab$elev = elev
          }
        }
        
        rm(listBreak.x) #now that originTab is created, we no longer need listBreak.x
        ...
        if(parallel == TRUE){
          ...
          distances<-foreach::foreach(j = date.vec, .packages = 'foreach') %dopar% day_listDistance(j, data.list, id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores)
          
        }else{ #if parallel == FALSE
          
          distances<-foreach::foreach(j = date.vec, .packages = 'foreach') %do% day_listDistance(j, data.list, id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores)
    ...
    list.dist<- foreach::foreach(k = 1:length(x), .packages = 'foreach') %do% listBreak_dist.generator(k,id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores, environmentTag = thisEnvironment) #we set the .packages argument to 'foreach' to allow us to use foreach loops within foreach loops
    ...
    if(parallel == TRUE){
      ...
      doParallel::registerDoParallel(cl)
      on.exit(parallel::stopCluster(cl))
      distances<-foreach::foreach(j = date.vec, .packages = 'foreach') %dopar% day_listDistance(j, data.list, id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores) #we set the .packages argument to 'foreach' to allow us to use foreach loops within foreach loops
      
    }else{ #if parallel == FALSE
      
      distances<-foreach::foreach(j = date.vec, .packages = 'foreach') %do% day_listDistance(j, data.list, id, dateTime, point.x, point.y, poly.xy, elev, parallel, dataType, lonlat, numVertices, nCores) #we set the .packages argument to 'foreach' to allow us to use foreach loops within foreach loops
