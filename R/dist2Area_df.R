...
dist2Area_df<-function(x = NULL, y = NULL, x.id = NULL, y.id = NULL, dateTime = NULL, point.x = NULL, point.y = NULL, poly.xy = NULL, parallel = FALSE, nCores = (parallel::detectCores()/2), dataType = "Point", lonlat = FALSE, numVertices = 4){
  if(is.data.frame(x) == FALSE & is.list(x) == TRUE){ #1/15 added the "is.data.frame(x) == FALSE" argument because R treats dataframes as lists.
      ...
      eval(expr = {
        ...
        dist.generator2<-function(x, y, x.id, y.id, dateTime, point.x, point.y, poly.xy, parallel, dataType, lonlat, numVertices, nCores){
          ...
          if(dataType == "point" || dataType == "Point" || dataType == "POINT"){
            ...
            rm(x) #now that originTab has been defined, we can safely remove x to free up local memory
        ...
        if(length(listBreak.x) > 0){ #for some reason using an "else" statement would always result in an originTab table with 0 records...
          if(length(x.id) > 0){ 
            if(length(x.id) == 1 & is.na(match(x.id[1], names(listBreak.x))) == FALSE){ #added 1/14 to accompany the list-processing functionality. If x is a list, rather than id being a vector of length(nrow(x)), it may be necessary to designate the colname for intended "id" values (i.e., if the ids in different list entries are different)
              listBreak.x$id <- listBreak.x[,match(x.id, names(listBreak.x))]
            }else{ #if length(id) > 1
              listBreak.x$id = x.id
            }
          }
        ...
        rm(listBreak.x) #now that originTab is made, there's no need to keep x. It can be safely removed to free up memory.
    ...
    list.dist<- foreach::foreach(k = 1:length(x), .packages = 'foreach') %do% listBreak_dist.generator2(k, y, x.id, y.id, dateTime, point.x, point.y, poly.xy, parallel, nCores, dataType, lonlat, numVertices, environmentTag = thisEnvironment) #we set the .packages argument to 'foreach' to allow us to use foreach loops within foreach loops
