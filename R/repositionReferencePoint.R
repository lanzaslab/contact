...
#' 
#' #Because the head is not the same width of the body and is assumed to be centered at the 
#'    #front of the body, before creating body polygons, we must move reference points (on the 
#'    #left ear) to the left by 0.3335 m to reposition them at the upper-left corner of calves 
#'    #bodies. Note that we are assuming left ears (where the tracking devices are located) are 
#'    #parallel to shoulder-tips. 
#' system.time(leftShoulder.point<-contact::repositionReferencePoint(x = calves.dateTime, 
#'    id = calves.dateTime$id, dateTime = calves.dateTime$dateTime, point.x = calves.dateTime$x, 
#'    point.y = calves.dateTime$y, direction = NULL, repositionAngle = 180, 
#'    repositionDist = 0.0835, immobThreshold = 0, parallel = FALSE, modelOrientation = 90)) 
...
repositionReferencePoint <- function(x = NULL, id = NULL, dateTime = NULL, point.x = NULL, point.y = NULL, direction = NULL, repositionAngle = 0, repositionDist = 1, immobThreshold = 0, parallel = FALSE, nCores = parallel::detectCores(), modelOrientation = 90){
  ...
  if(is.data.frame(x) == FALSE & is.list(x) == TRUE){ #02/02/2019 added the "is.data.frame(x) == FALSE" argument because R apparently treats dataframes as lists.
    ...
    list.rep <- apply(breakFrame, 1, list.breaker,y = x, id, dateTime, point.x, point.y, direction, repositionAngle, repositionDist, immobThreshold, parallel, modelOrientation, nCores) #in the vast majority of cases, parallelizing the subfunctions will result in faster processing than parallelizing the list processing here. As such, since parallelizing this list processing could cause numerous problems due to parallelized subfunctions, this is an apply rather than a parApply or lapply.

    return(list.rep)

  }else{ #if x is a dataFrame

    frame.rep<- reposition.generator(x, id, dateTime, point.x, point.y, direction, repositionAngle, repositionDist, immobThreshold, parallel, modelOrientation, nCores)
