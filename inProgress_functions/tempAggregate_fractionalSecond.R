...
tempAggregate <- function(x = NULL, id = NULL, point.x = NULL, point.y = NULL, dateTime = NULL, secondAgg = 10, extrapolate.left = FALSE, extrapolate.right = FALSE, resolutionLevel = "full", parallel = FALSE, nCores = (parallel::detectCores()/2), na.rm = TRUE, smooth.type = 1) { #removed totalSecond = NULL argument on 01102019
  ...
  Agg.generator<-function(x, id, point.x, point.y, dateTime, secondAgg, extrapolate.left, extrapolate.right, resolutionLevel, parallel, na.rm, smooth.type, nCores){
    ...
    originTab$totalSecond <- difftime(originTab$dateTime ,originTab$dateTime[1] , units = c("secs")) #calculates the total second of each timepoint in x
    
    totalObs<-foreach::foreach(i == unique(originTab$id)) %do% { #we define the unique sequential observations for each individual
      idSub<- droplevels(subset(originTab, id == i))
      idSub$observationSeq <- seq(1:nrow(idSub))
      return(idSub)
    }
    
    originTab.totalObs <- data.frame(data.table::rbindlist(totalObs), stringsAsFactors = TRUE) #bind these data together again
    originTab<-originTab.totalObs[order(originTab.totalObs$totalSecond),]  #reorder the data by totalSecond again
    rm(list = c("totalObs", "originTab.totalObs")) #remove totalObs to free up local memory.
    ...
    Processing_and_Aggregation_Procedures<-function(brk.point, originTab, extrapolate.left = leftExtrap, extrapolate.right = rightExtrap, resolutionLevel, smooth.type, secondAgg){
      ...
      if(secondAgg >= 1){
        ...
        #if secondAgg is less than 1, we multiply the number of vectors by the number of secondAgg fractional seconds in a single second. Note: if secondAgg < 1, 1 MUST be evenly divisible by secondAgg
        coord.tmp<-matrix(rep(0,nDays*24*60*60*(1/secondAgg)*2),ncol=2) #create a matrix populated by zeroes for x and y coordinates at every second within the time span
        secondAgg1<-(1/secondAgg) #update the number of observations to pull during interpolation
      }
      ...
      n=data.one[1,match("observationSeq", names(data.one))] # get current position at 1st data daily timepoint (previous point)
      ...
      if(extrapolate.left == TRUE){ #This section extrapolates the left tails of individuals' paths by filling in the missing time between the first recorded second and the first second of the day. It assumes that the individual starts the first second of the day at the same place it spent the first recorded second.
        ...
        if(n > 1){ #Note that n cannot be smaller than "1," as "observationSeq" calculated above cannot be smaller than 1.
      ...
      if(data.len > 1){
        for (i in 2:data.len) { #this grabs the second position 1:length of the subset; that's because it moves from the first second to last one in the set looking for holes
          n.current=data.one[i,match("observationSeq", names(data.one))] #this grabs the next observation from the subset
    ...
    brk.point<-cbind(start.brk,end.brk,seq(1, length(end.brk),1), secondAgg) #these two vectors offset beginning and endpoints for observations from different individuals.
