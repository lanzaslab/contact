RTLS_SensSpec<-function(inContact.n = 1000, outContact.n = NULL, acc.Dist1 = 0.5, acc.Dist2 = NULL, pWithin1 = 90, pWithin2 = NULL, spTh = 0.666, outContact.range = c(0.667, 1.666)){
  ...
  if(is.na(outContact.range[2]) == TRUE){ #if the maximum out-of-contact distance is not specified, the maximum distance updates to acc.Dist1 + acc.Dist2 and users are informed.
    
    outContact.range[2] = spTh + acc.Dist1 + acc.Dist2 #we chose this value because it is the maximum value that two out-of-contact points could be away from one another while plausibly being observed "in-contact" with one another due to positioning errors.
    warning("Maximum distance for out-of-contact points is unspecified. Defaulting to spTh + (acc.Dist1 + acc.Dist2).", immediate. = TRUE)
